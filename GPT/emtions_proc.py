...
if 'Responses' in book.sheetnames:
    ...
    sheet = book.create_sheet('Emotions')
        ...
        messages=[
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": """Identify the emotions that are detected in this photo, labeling as 1 or 0 for each of these identified emotions: (use "-" to annotate 1 or 0 next to the label name: e.g. Focused - 1) If there arent any human figures or faces, just label "0" for all of them. (Also, just labelling them, no need further explanation for each action.) [
                    "Focused",
                    "Determined",
                    "Tired",
                    "Alert",
